 ...
 * @param {Object} res - The response object
 * @returns {Promise<void>}
 * @throws {ApiError} - If the borrow rejection fails
 */
const rejectBorrow = async (req, res, next) => {
  ...
  try {
    const updatedBorrow = await borrowService.rejectBorrow(userId, borrowId);
    res.status(200).json({
...
const getAllBorrows = async (req, res, next) => {
  try {
    ...
    const borrows = await borrowService.getAllBorrows(filters, skip, limit);
    res.status(200).json({
